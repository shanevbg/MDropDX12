...
Note that you can choose to hide either the Tabs or the Button panel with the popup menu that opens by clicking "MDropDX12" in the bottom right corner. You may also use the menu to access help resources, open the Visualizer window, switch between light and dark mode, and toggle color or monochrome button image display.
...
MDropDX12 Visualizer is a ground-up DirectX 12 rebuild of the MilkDrop2 rendering engine, with GPU-accelerated Direct2D text overlays, a rewritten texture pipeline, and an in-app settings UI. While the codebase has been extensively rewritten, full preset compatibility with the MilkDrop2 ecosystem is maintained. New variables and features have been introduced to give preset authors and users more possibilities while keeping presets compatible with other MilkDrop2 based visualizers (eg. NestDrop, BeatDrop Music Visualizer or MilkDrop 3).

## Settings Window (F2)

Press F2 (or Ctrl+L) to open the in-app Settings window. It provides a 5-tab interface with dark theme support:

**General** tab: Audio device selection, preset directory browser with preset list, forward/back navigation buttons, and general options (sequential preset order, hard cuts, preset lock, etc.). The "Resources..." button opens the Resource Viewer showing all textures loaded by the current preset with their load status, type, dimensions, and file paths.

**Visual** tab: Display options including FPS display, always-on-top, and render quality. Reset button restores defaults.

**Colors** tab: Hue, Saturation, and Brightness sliders for live color shifting. Gamma slider with auto-reset for low-gamma presets. Reset button restores defaults.

**Sound** tab: Audio-related settings.

**Files** tab: Configure fallback search paths for textures. When a preset references a texture that isn't found in the built-in directory, these paths are searched in order. A dedicated "Random Textures Directory" setting provides a separate directory specifically for random texture selection (rand00-rand13), which takes priority over fallback paths.

### Keyboard Shortcuts

| Key | Action |
| --- | ------ |
| F2 / Ctrl+L | Open Settings window |
| B / b | Increase / decrease brightness |
| G / g | Increase / decrease gamma |
| R | Toggle random / sequential preset order |
| Scroll Lock | Toggle preset lock |
