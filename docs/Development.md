...
## Milk / milk2 coordinate spaces

DX9 used `OrthoLH(2,-2)`, which negated Y. The old `ShowToUser_*`, `DrawCustomWaves`, and `DrawCustomShapes` functions are still in `milkdropfs.cpp` but are **not called** on the DX12 path. The live code is `DX12_*`. Do not copy DX9’s `y * -2 + 1` into DX12 without Ortho — that pair is the same visual as `y * 2 - 1` on a passthrough VS.

Three spaces:

1. **EEL** — `x,y` in `[0,1]`. Warp per-vertex uses `pv_y = clip_y * -0.5 + 0.5` (mesh `y=0` at the top). Custom-wave docs use `y=0` at the bottom. Those two conventions are real; do not “fix” one by flipping the other.
2. **DX12 clip** — `(-1,-1)` bottom-left, `(+1,+1)` top-right, passthrough VS.
3. **VS / shader UV** — D3D `uv.y=0` is the top of the render target. Comp folds and Split Mode see this.

| Path | EEL → clip |
| --- | --- |
| Custom waves / shapes (DX12) | `x*2-1`, `y*2-1` |
| Sprites → backbuffer | build quad, then `y *= -1` (Feb Ortho compensation) |
| Sprites → VS (milk2 layers 1–4) | draw into VS before comp (Split Mode / folds see them). `SpriteSX/SY` **magnitude is size** (a leading minus is not a flip). Recenter the quad on `clip y=0` so the image sits on the midline. Flip texture **V only** (`v=0` is the top of the RT). Do not flip clip Y on this path — that slides the pair above/below the line. |
| Warp mesh | do not change `pv_y` |

`.milk2` is always two states (PRESET1 = old, PRESET2 = new) with a frozen wipe. `blending_direction` is wipe orientation; do not rewrite progress unless a side-by-side with MilkDrop 3 proves it.

The DX9 `ShowToUser_NoShaders` path still has the **video-echo / gamma / brighten-darken** passes. Custom-comp presets never got those on DX12. Port that behavior as a post-comp pass, do not invent per-preset flips.
...
1. **Signals** — prefixed with `SIGNAL|`, dispatched via `DispatchSignal()` which posts a `WM_APP + offset` message to the render window. Signal table is in `pipe_server.cpp`:
   ...
   | `SIGNAL\|MIRROR_INDEPENDENT` | Toggle independent per-display mirror render |
   | `SIGNAL\|ALWAYS_ON_TOP` | Toggle always-on-top (primary + mirrors) |
...
2. **Commands** — everything else is forwarded to `LaunchMessage()` in `engine_messages.cpp`. Commands are matched via `wcsncmp` and include:
   ...
   | `SET_IDLE_TIMER=0\|1` | Enable/disable the idle timer (persists; restores first if idle is active) |
   | `GET_IDLE_TIMER` | Query idle timer — replies `IDLE_TIMER=<enabled>\|<timeoutMin>\|<action>` |
   | `TESTING_MODE=0\|1` | Freeze automatic preset changes and ignore the keyboard, so a measurement cannot be altered mid-run. Query with no value. Never persisted; ESC twice leaves it |
   | `PRESET_OVERRIDE_STATUS` | Per-slot resolution for the running preset — replies `PRESET_OVERRIDE\|shader=..\|shaderSrc=none\|rule\|preset\|vfx=..\|vfxSrc=..\|tag=..` |
   | `SET_PRESET_SHADER_OVERRIDE=<name>` | The running preset's own shader override; an empty value means "explicitly none" |
   | `SET_PRESET_VFX_PROFILE=<name>` | The running preset's own VFX profile; an empty value means "explicitly none" |
   | `CLEAR_PRESET_OVERRIDE=shader\|vfx` | Remove the member so the preset's tags apply again (different from setting it empty) |
   | `VFX_SCOPED_STATUS` | Replies `VFX_SCOPED\|active=..\|profile=..\|dirty=..` |
   | `VFX_SCOPED_KEEP=0\|1` | Answer the Keep/Discard prompt without clicking; 1 writes the edit into the scoped profile |
   | `TESTING_MODE=0\|1` | Testing mode: stop every automatic preset change and ignore the keyboard. Not persisted. Reply `TESTING_MODE=<0\|1>` |
   | `TESTING_MODE` | Query testing mode |
...
A measurement is only worth the guarantee that what was measured is what was
loaded, and that guarantee did not exist: during the MD31 shader work a
constant readout was taken against a different preset, because the preset
auto-advanced mid-measurement. The reading was quietly meaningless.

`TESTING_MODE=1` stops everything that changes the frame without being asked:

* the timed preset advance,
* the audio-triggered hard cuts (`HardcutMode` 2-8),
* the preset change on song change,
* the idle timer,
* the keyboard and mouse-button bindings.

Explicit commands are untouched — `PRESET=`, `NEXT_PRESET`, `ACTION=` and the
shader override commands all still work. The point is to remove what the
harness did not ask for, not to freeze the app.

It also keeps the run from taking your settings and your preset database with
it:

* **Tool windows open on the render window's monitor**, rather than at their
  remembered positions, so a run's windows stay with the thing being measured
  instead of landing on whichever display they were last used on.
* **Tool window positions are not saved.** Outside testing mode a window
  remembers where it was left; during a run that would make the harness's
  placement permanent, and you would come back to every window moved.
* **Nothing is written to `presets.json`** — no new entries, no play counts, no
  times, no auto-captured shader errors. A run parks one preset on screen for
  minutes and steps through hundreds of them, which would otherwise make "most
  played" a report on the test rig.

Writes you explicitly ask for still go through: `SET_PRESET_RATING` and
`SET_PRESET_NOTE` record normally even in testing mode. The rule is
automatic-versus-explicit, not "everything stops".

Note that this does **not** cover `settings.ini` as a whole — configuration
writes happen at several hundred call sites across the source and are not
funnelled through one function, so a run that changes a setting still persists
it. Only the window-position writes above are suppressed.

**It is a set, not a toggle.** `ACTION=LockPreset` toggles, which a script
cannot drive without already knowing the state. `TESTING_MODE=1` twice leaves
it on.

**It is never written to the INI**, so it cannot survive a restart — the same
rule the VFX profile store settled on.

To leave it from the keyboard, press **ESC twice**. One press only warns: a
stray key must not end a run that has been going for minutes.

Two traps worth knowing when driving any of this over the pipe:

* `SEND=` posts `WM_KEYDOWN` only for a **hex** keycode. `SEND=27` goes down
  the `WM_CHAR` branch and types `2` then `7`. Use `SEND=0x1B`.
* Every command that replies must have its reply **read**, or it stays buffered
  and the next read returns the stale value.
