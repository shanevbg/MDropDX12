...
- **[GLSL Sandbox](https://glslsandbox.com)** — simpler shaders, mostly single-pass fullscreen effects. Good for getting started.
  - **CLI import:** `python tools/glslsandbox_to_milk3.py https://glslsandbox.com/e#109677.0` fetches the effect and writes a `.milk3` under `resources/presets/`. Use `-o path` to choose the output file. Conversion is best-effort (Sandbox `time`/`resolution`/`mouse`/`backbuffer` → milk3 HLSL); open Shader Import if compile fails.
