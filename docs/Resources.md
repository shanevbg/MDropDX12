...
MDropDX12 uses `.milk` / `.milk3` preset files and texture images to generate visuals. The exe is self-bootstrapping — it creates the `resources/presets/` and `resources/textures/` directories automatically on first run. Portable releases include one redistributable demo preset (`sb_tunnel.milk3`); larger community collections are still optional.
