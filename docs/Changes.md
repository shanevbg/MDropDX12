...
- **Removed the MD3 Glow halo and the Martin string ribs**, along with the
  Video Effects window's Rendering tab, the `Md3GlowStrength` / `MartinRibCore`
  / `MartinRibHalo` settings and the `SET_TUNABLE` / `GET_TUNABLES` commands.
  Both effects were invented to stand in for the shader an `MD31`/`MD32` preset
  does not carry, and neither is in MilkDrop 3's frame — on Rainbow Butterfly1
  they drew five bright ribs and a halo over the preset. Use a custom shader
  override to supply a real warp shader instead. A `rendering` block in an
  existing VFX profile is left in place rather than stripped.
