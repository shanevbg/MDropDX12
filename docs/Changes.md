...
Independent multi-monitor mirrors, `.milk2` transition accuracy measured against
MilkDrop 3 PRO, user-owned shader overrides, and content-hash preset identity.
...
### `.milk2` double-preset transitions

Wipe geometry is now measured against MilkDrop 3 PRO rather than guessed. Weighted
mean absolute error across real files fell from **0.4668 to 0.0531**.

- **All 26 wipe patterns recovered** by measurement, with the six that have no closed form baked as fields and the C++ table generated from the measurements.
- **MilkDrop's own wipe parameterisation** adopted, confirmed against a Ghidra decompilation showing MD3's transitions are MilkDrop's own.
- **All eleven sprite blend modes** now match MD3, including modes 6-10 and the removal of two clamps.
- Sprite `SpriteX`/`SpriteY` offsets were parsed and dropped; now applied, and no longer double-applied. Sprite scale is the sum of header and code values, keeping the header's sign.
- Fixed `blending_direction` handling, wipe Y-orientation, synthesised borders, and a frozen `.milk2` surviving a device teardown.
...
- An **open-source importer** (`tools/md3_shader_import.py`) converts a licensee's own MD3 PRO shader cache to DX12 HLSL. It ships no MD3 data.
