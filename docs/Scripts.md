...
Engine fidelity knobs, also on the **Rendering** tab of the Video Effects
window. They sit there rather than in the Visual window (which is about
performance, memory and GPU tuning) because they are saved into the VFX
profile, and the profile's Save button is on that window.

The IPC name is the same string as the `[Settings]` INI key. A `SET_TUNABLE`
writes that key and the live-state JSON, so the value survives a restart and is
not lost when the app rewrites `settings.ini` on exit.

Values out of range are clamped, not rejected; the response reports what was
actually stored.

| Command | Response | Description |
|---------|----------|-------------|
| `GET_TUNABLES` | `TUNABLES=<Name>=<value>,<min>,<max>\|…` | List every tunable with its value and range |
| `SET_TUNABLE=<Name>=<int>` | `TUNABLE=<Name>=<value>` | Set one tunable. Unknown name returns `TUNABLE=ERROR\|unknown\|<Name>` |

| Tunable | Default | Range | Meaning |
|---------|---------|-------|---------|
| `Md3GlowStrength` | 55 | 0–300 | Halo strength for presets carrying `MD31`/`MD32`, in hundredths (0 = off) |
| `MartinRibCore` | 14 | 0–300 | Half-width in pixels of the bright core of Martin string ribs |
| `MartinRibHalo` | 60 | 0–300 | Half-width in pixels of the soft halo around those ribs |
...
Profiles also carry the **render tunables** (`GET_TUNABLES` above) in a
`rendering` object, so a saved look restores its glow and rib widths too. A
profile written before that existed simply has no `rendering` key, and loading
it leaves the current tunables alone rather than resetting them.
...
| Shape | Becomes |
|---|---|
| `settings.ini` with `[VideoFX]` parameter keys (plus `[Settings]` tunables) | one profile, named by you |
...
> `SET_VFX`, `SET_TUNABLE` and `VFX_RESET` change live state and write nothing
