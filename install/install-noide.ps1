# install-noide.ps1 - Set up MDropDX12 build environment (no IDE)
# Usage: powershell -ExecutionPolicy Bypass -File install-noide.ps1
#
# Installs Git, VS 2022 Build Tools (MSVC v143, MSBuild, Windows 11 SDK),
# clones the repo, and runs a Release x64 build. No editor/IDE is installed.
#
...
Write-Step "MDropDX12 Build Environment Setup (no IDE)"
Write-Host "  This script installs Git and VS 2022 Build Tools,"
...
if (-not $hasWinget) {
    ...
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        ...
        Write-Host "  WARNING: Could not install winget." -ForegroundColor Yellow
    }
} else {
