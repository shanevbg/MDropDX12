...
typedef struct _compileContext
{
  ...
  // MDropDX12: per-VM base for reg00-99 (NULL = legacy process-global
  // __nseel_global_regs). Resolved at COMPILE time in get_global_var(), so it
  // must be set before NSEEL_code_compile — same rule as gram_blocks. All VMs
  // of one render context share one base so per_frame/per_pixel/shape/wave
  // sections keep communicating through regXX within that context.
  EEL_F *reg_base;
