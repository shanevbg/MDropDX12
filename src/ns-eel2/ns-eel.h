...
// MDropDX12: per-VM base for reg00-99 (EEL1 compat regs). base100 points at
// 100 EEL_Fs owned by the caller; NULL = legacy process-global regs.
// Must be set before compilation (addresses bake at compile time).
void NSEEL_VM_SetRegBase(NSEEL_VMCTX ctx, EEL_F *base100);
...
#define NSEEL_EEL1_COMPAT_MODE // MilkDrop presets use legacy EEL1 functions: megabuf(), gmegabuf(), if(), assign(), etc.
