...
opcodeRec *nseel_resolve_named_symbol(compileContext *ctx, opcodeRec *rec, int parmcnt, int *errOut)
{
  ...
  if (prevent_function_calls)
  {
    ...
    else if (!stricmp(sname,"int"))
    {
      sname = "floor"; // MilkDrop compat: int() is an alias for floor()
    }
    else
#endif
...
EEL_F *get_global_var(compileContext *ctx, const char *gv, int addIfNotPresent)
{
  ...
  {
    // MDropDX12: honor the per-VM reg base when set (independent mirror
    // simulations); NULL falls back to the legacy shared array.
    return (ctx && ctx->reg_base ? ctx->reg_base : __nseel_global_regs) + atoi(gv+3);
  }
#endif
