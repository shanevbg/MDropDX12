...
void Engine::LoadSpoutInputSettings()
{
    ...
    // that kept them here.
    const bool bMovedToJson = GetPrivateProfileIntW(L"VideoFX", L"MovedToJson", 0, pIni) != 0;
    ...
    GetPrivateProfileStringW(L"VideoFX", L"szVFXStartup", L"", m_szVFXStartup, MAX_VFX_PROFILE_NAME, pIni);
    ...
    m_bEnableVFXSaveOnExit = GetPrivateProfileIntW(L"VideoFX", L"bEnableVFXSaveOnExit", 0, pIni) != 0;
    ...
    if (!bMovedToJson)
        WritePrivateProfileStringW(L"VideoFX", L"MovedToJson", L"1", pIni);
...
void Engine::SaveSpoutInputSettings()
{
    ...
    WritePrivateProfileStringW(L"VideoFX", L"bEnableVFXSaveOnExit", m_bEnableVFXSaveOnExit ? L"1" : L"0", pIni);
    ...
    WritePrivateProfileStringW(L"VideoFX", L"szCurrentVFXProfile", NULL, pIni);
    WritePrivateProfileStringW(L"VideoFX", L"bEnableVFXStartupSavingOnClose", NULL, pIni);
}
