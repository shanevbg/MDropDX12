...
JsonValue VFXProfileFromIni(const wchar_t* path, const TunableList& tun)
{
    ...
    for (const wchar_t* k : kProbe) {
        GetPrivateProfileStringW(L"VideoFX", k, L"", probe, 64, path);
    ...
    GetPrivateProfileStringW(L"VideoFX", L"WndW", L"", buf, 64, path);
    ...
    auto readF = [&](const wchar_t* key, float def) -> float {
        GetPrivateProfileStringW(L"VideoFX", key, L"", buf, 64, path);
        return buf[0] ? (float)_wtof(buf) : def;
    };
    auto readB = [&](const wchar_t* key, bool def) -> bool {
        return GetPrivateProfileIntW(L"VideoFX", key, def ? 1 : 0, path) != 0;
    };
    ...
    for (const auto& t : tun) {
        GetPrivateProfileStringW(L"Settings", t.first.c_str(), L"", buf, 64, path);
    ...
    int blend = GetPrivateProfileIntW(L"VideoFX", L"BlendMode", 0, path);
    ...
    auto writeAR = [&](const wchar_t* key, const wchar_t* prefix) {
        ...
        int src = GetPrivateProfileIntW(L"VideoFX", k, 0, path);
