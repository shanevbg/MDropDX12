...
void ButtonPanel::SaveToINI(const wchar_t* iniPath, const wchar_t* section) const {
    ...
    ConfigFile(iniPath).SetString(section, L"Rows", buf);
    swprintf(buf, 32, L"%d", m_config.nCols);
    ConfigFile(iniPath).SetString(section, L"Cols", buf);
    swprintf(buf, 32, L"%d", m_config.nBanks);
    ConfigFile(iniPath).SetString(section, L"Banks", buf);
    swprintf(buf, 32, L"%d", m_config.nActiveBank);
    ConfigFile(iniPath).SetString(section, L"ActiveBank", buf);
    swprintf(buf, 32, L"%d", m_config.bScrollable ? 1 : 0);
    ConfigFile(iniPath).SetString(section, L"Scrollable", buf);

    int total = m_config.nBanks * m_config.nRows * m_config.nCols;
    swprintf(buf, 32, L"%d", total);
    ConfigFile(iniPath).SetString(section, L"SlotCount", buf);
    ...
    for (int i = 0; i < total && i < (int)m_slots.size(); i++) {
        ...
        ConfigFile(iniPath).SetString(section, key, val.c_str());

        // Save image path if present
        if (!s.imagePath.empty()) {
            wchar_t imgKey[32];
            swprintf(imgKey, 32, L"SlotImg_%d", i);
            ConfigFile(iniPath).SetString(section, imgKey, s.imagePath.c_str());
        }
        ...
        if (s.vk != 0) {
            ...
            ConfigFile(iniPath).SetString(section, hkKey, hkVal);
        }
    }
}

void ButtonPanel::LoadFromINI(const wchar_t* iniPath, const wchar_t* section) {
    m_config.nRows       = ConfigFile(iniPath).GetInt(section, L"Rows", 3);
    m_config.nCols       = ConfigFile(iniPath).GetInt(section, L"Cols", 5);
    m_config.nBanks      = ConfigFile(iniPath).GetInt(section, L"Banks", 3);
    m_config.nActiveBank = ConfigFile(iniPath).GetInt(section, L"ActiveBank", 0);
    m_config.bScrollable = ConfigFile(iniPath).GetInt(section, L"Scrollable", 0) != 0;
    ...
    int total = ConfigFile(iniPath).GetInt(section, L"SlotCount", -1);
    if (total < 0) {
    ...
    for (int i = 0; i < total && i < (int)m_slots.size(); i++) {
        ...
        swprintf(key, 32, L"Slot_%d", i);
        ConfigFile(iniPath).GetStringTo(section, key, L"", val, 2048);
        if (val[0] == 0) continue;
        ...
        ConfigFile(iniPath).GetStringTo(section, imgKey, L"", imgVal, MAX_PATH);
        if (imgVal[0])
        ...
        swprintf(hkKey, 32, L"SlotHK_%d", i);
        wchar_t hkVal[128];
        ConfigFile(iniPath).GetStringTo(section, hkKey, L"", hkVal, 128);
        if (hkVal[0]) {
