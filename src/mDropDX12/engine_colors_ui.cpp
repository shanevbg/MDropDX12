...
void ColorsWindow::DoBuildControls() {
  ...
  TrackAnchored(CreateSlider(hw, IDC_MW_COL_HUE, x + lw + 4, y, sliderW, lineH, 0, 200, (int)(p->m_ColShiftHue * 100) + 100), kAnchorStretchWide);
  ...
  TrackAnchored(CreateSlider(hw, IDC_MW_COL_SAT, x + lw + 4, y, sliderW, lineH, 0, 200, (int)(p->m_ColShiftSaturation * 100) + 100), kAnchorStretchWide);
  ...
  TrackAnchored(CreateSlider(hw, IDC_MW_COL_BRIGHT, x + lw + 4, y, sliderW, lineH, 0, 200, (int)(p->m_ColShiftBrightness * 100) + 100), kAnchorStretchWide);
  ...
  TrackAnchored(CreateSlider(hw, IDC_MW_COL_GAMMA, x + lw + 4, y, sliderW, lineH, 0, 80, (int)(p->m_pState->m_fGammaAdj.eval(-1) * 10)), kAnchorStretchWide);
  ...
  // "Sec(s)" rather than "Seconds:" -- the label shares its row with the Auto
  // Hue checkbox and the value edit, and the longer word was being clipped to
  // "Secon". The width and the edit's offset are font-scaled with the usual
  // MulDiv(N, lineH, 26) idiom so it stays legible at every font size, not just
  // the default one; as raw pixels they clipped again the moment the user
  // pressed the font + button.
  const int secLabelW = MulDiv(60, lineH, 26);
  TrackControl(CreateLabel(hw, L"Sec(s):", x + rw / 2, y, secLabelW, lineH, hFont));
  swprintf(buf, 64, L"%.3f", p->m_AutoHueSeconds);
  TrackControl(CreateEdit(hw, buf, IDC_MW_AUTO_HUE_SEC, x + rw / 2 + secLabelW + 4, y,
                          MulDiv(70, lineH, 26), lineH, hFont));
  y += lineH + gap + 4;
