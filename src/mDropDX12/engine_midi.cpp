...
{
  m_bMidiEnabled = Config().GetInt(L"MIDI", L"Enabled", 0) != 0;
  m_nMidiDeviceID = Config().GetInt(L"MIDI", L"DeviceID", -1);
  Config().GetStringTo(L"MIDI", L"DeviceName", L"", m_szMidiDeviceName, 256);
  m_nMidiBufferDelay = Config().GetInt(L"MIDI", L"BufferDelay", 30);
}
...
void Engine::SaveMidiSettings()
{
  ...
  Config().SetString(L"MIDI", L"Enabled", m_bMidiEnabled ? L"1" : L"0");
  swprintf(buf, 32, L"%d", m_nMidiDeviceID);
  Config().SetString(L"MIDI", L"DeviceID", buf);
  Config().SetString(L"MIDI", L"DeviceName", m_szMidiDeviceName);
  swprintf(buf, 32, L"%d", m_nMidiBufferDelay);
  Config().SetString(L"MIDI", L"BufferDelay", buf);
}
