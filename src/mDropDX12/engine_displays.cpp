...
void Engine::MirrorThreadMain()
{
    ...
    while (!m_bMirrorThreadQuit.load()) {
        WaitForSingleObject(m_hMirrorWake, 50);
...
{
    // Opposite-orientation render only. Does not Present. Same-orient mirrors
    // stretch/copy the primary on the main thread.
    ...
    // Mirrors are not frame-locked to the primary, so cap the worker rather than
    // let it contend every wake. ~30 fps is plenty for a mirror panel and keeps
    // the primary's stall (it blocks while we hold the lock) bounded.
    const DWORD nowTick = GetTickCount();
    if (m_dwLastOrientTick != 0 && (nowTick - m_dwLastOrientTick) < 33)
        return;

    // Announce intent, then block. try_to_lock lost the race essentially always.
    m_bMirrorWorkerWantsLock.store(true, std::memory_order_release);
    std::unique_lock<std::mutex> lk(m_mirrorEngineMutex);
    m_bMirrorWorkerWantsLock.store(false, std::memory_order_release);
    m_dwLastOrientTick = nowTick;
...
void Engine::SendToDisplayOutputs()
{
    ...
    for (auto& out : m_displayOutputs) {
        ...
        int tw = (lw > 0) ? lw : out.monitorState->width;
        int th = (lh > 0) ? lh : out.monitorState->height;
    ...
    const bool orientReady = anyOppositeIndep && m_bOrientImageReady.load() &&
        m_orientPipe.ready && workerIdle && m_orientPipe.frames >= 2;
    ...
    for (auto& out : m_displayOutputs) {
        ...
        if (doCopy) {
            ...
            if (useOrient) {
                ...
                // Opposite independent without a ready landscape pass: stretch-fill
                // (letterbox looked like an unstretched portrait copy).
