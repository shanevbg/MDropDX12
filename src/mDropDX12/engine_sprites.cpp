...
bool Engine::LaunchMilk2Sprite(const Milk2SpriteDesc& spr) {
  ...
  if (img[1] != L':') {
    ...
    auto tryPath = [&](const wchar_t* base, const wchar_t* rel) {
      ...
      swprintf_s(cand, L"%s%s", base, rel);
      if (fileExists(cand))
        wcscpy_s(img, cand);
    };
    ...
    if (leaf && leaf[0]) {
      ...
      swprintf_s(spritesRel, L"sprites\\%s", leaf);
      swprintf_s(texturesRel, L"textures\\%s", leaf);
    ...
    if (!img[0]) {
      ...
      if (m_szContentBasePath[0])
        swprintf_s(img, L"%s%s", m_szContentBasePath, temp);
      else
  ...
  if (!spr.szInit.empty()) {
    strcat_s(initcode, "\x01");
    strcat_s(initcode, spr.szInit.c_str());
  }
  ...
  if (fabsf(spr.fSpeed) > 1.0e-6f) {
    char spin[80];
    sprintf_s(spin, "rot=rot+%.6f/fps;", spr.fSpeed);
    strcat_s(code, spin);
  }
