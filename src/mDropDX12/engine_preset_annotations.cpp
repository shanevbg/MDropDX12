...
void Engine::ResolveShaderOverrideForPreset(CState* pState)
{
    ...
    if (!a || a->tags.empty()) return;

    std::wstring matchedTag;
    const ShaderOverride* o = store.Resolve(a->tags, &matchedTag);
    if (!o) return;

    m_activeOverride.name       = o->name;
    m_activeOverride.warpText   = o->warpText;
    m_activeOverride.compText   = o->compText;
    m_activeOverride.fromRule   = true;
    m_activeOverride.matchedTag = matchedTag;

    DLOG_INFO("ShaderOverride: '%ls' applies (tag '%ls'), warp=%d comp=%d bytes",
              o->name.c_str(), matchedTag.c_str(),
              (int)o->warpText.size(), (int)o->compText.size());
}
