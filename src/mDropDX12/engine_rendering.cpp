...
void Engine::RenderInjectEffect()
{
  ...
  UINT presetFxMask = 0;
  const bool classicPost = !m_dx12CompPSO
      || (m_pState && m_pState->CompShaderIsGetPixelPassthrough());
  ...
  // Glow for MD31/MD32 presets -- see the shader in engine.cpp for why.
  UINT glow = 0u;
  if (m_pState && m_pState->m_bMD3CachedShaderPreset)
    glow = (UINT)m_nMd3GlowStrength;

  if (m_nInjectEffectMode == 0 && presetFxMask == 0 && srgbGamma == 0 && glow == 0) return;
  ...
  struct { UINT mode[4]; } cbData = { { (UINT)m_nInjectEffectMode, presetFxMask, srgbGamma, glow } };
