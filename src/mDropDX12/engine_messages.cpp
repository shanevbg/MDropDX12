...
void Engine::LaunchMessage(wchar_t* sMessage) {
  ...
  else if (wcsncmp(sMessage, L"SET_TUNABLE=", 12) == 0) {
    // Set one render tunable.  Format: SET_TUNABLE=<Name>=<int>
    // Names and ranges come from kRenderTunables (render_tunables.h) and are
    // the same strings used as [Settings] INI keys.
    extern PipeServer g_pipeServer;
    std::wstring name, value;
    if (!SplitNameValue(sMessage + 12, name, value)) {
      g_pipeServer.Send(L"TUNABLE=ERROR|expected SET_TUNABLE=<Name>=<value>");
    } else {
      const RenderTunable* found = nullptr;
      for (int i = 0; i < kRenderTunableCount; i++)
        if (_wcsicmp(kRenderTunables[i].name, name.c_str()) == 0) { found = &kRenderTunables[i]; break; }
      if (!found) {
        g_pipeServer.Send(L"TUNABLE=ERROR|unknown|" + name);
      } else {
        const int applied = ApplyRenderTunable(this, *found, _wtoi(value.c_str()));
        // Tunables are part of the VFX profile now, so a change has to go
        // through the same path as a slider: write live state, update the
        // Save button. ApplyRenderTunable only writes the INI fallback.
        OnVideoFXChanged();
        wchar_t buf[256];
        swprintf_s(buf, L"TUNABLE=%s=%d", found->name, applied);
        g_pipeServer.Send(buf);
        RepaintToolWindow(m_pVideoEffectsWindow);
        DLOG_INFO("Render tunable set via IPC: %S=%d", found->name, applied);
      }
    }
  }
  else if (wcsncmp(sMessage, L"GET_TUNABLES", 12) == 0) {
    // Report every render tunable as <Name>=<value>,<min>,<max>|...
    std::wstring out = L"TUNABLES=";
    for (int i = 0; i < kRenderTunableCount; i++) {
      const RenderTunable& t = kRenderTunables[i];
      wchar_t buf[256];
      swprintf_s(buf, L"%s%s=%d,%d,%d", (i ? L"|" : L""), t.name,
                 this->*(t.member), t.minValue, t.maxValue);
      out += buf;
    }
    extern PipeServer g_pipeServer;
