...
void CustomShadersWindow::DoBuildControls()
{
    ...
    const int ruleBtnY   = bottom - lineH;
    // The shader list is the primary one, so the rules list takes the smaller
    // share of what is left.
    int ruleH            = (bottom - y) / 4;
    if (ruleH < lineH * 3) ruleH = lineH * 3;
    const int ruleListY  = ruleBtnY - gap - ruleH;
    const int ruleLblY   = ruleListY - gap - lineH;

    const int shaderBtnY = ruleLblY - gap - row * 4;   // four rows of buttons
    const int listY      = y + row;                    // after the "Shaders:" label
    int listH            = shaderBtnY - gap - listY;
