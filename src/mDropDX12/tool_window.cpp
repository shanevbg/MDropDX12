...
void ToolWindow::LoadWindowPosition() {
  ...
  // Read as text, not with GetPrivateProfileIntW: that one returns zero for any
  // negative value, and a monitor placed left of or above the primary gives
  // windows negative coordinates. Saving x=-1800 and reading back 0 is how a
  // window on the left-hand screen jumps to the primary one on restart.
  auto readPos = [&](const wchar_t* key) -> int {
    wchar_t buf[32] = {};
    GetPrivateProfileStringW(sec, key, L"", buf, 32, ini);
    return buf[0] ? _wtoi(buf) : INT_MIN;
  };
...
LRESULT CALLBACK ToolWindow::BaseWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_SIZE:
    if (wParam != SIZE_MINIMIZED) {
      ...
      // needs a relayout.
      if (w == tw->m_nWndW && h == tw->m_nWndH) return 0;
      tw->m_nWndW = w;
      tw->m_nWndH = h;
      tw->OnResize();
    }
