...
void DisplaysWindow::BuildOutputsPage(int x, int y, int rw, int lineH, int gap) {
  ...
  // Independent mirror FPS cap (indented under the Independent render checkbox).
  // Parity = free-run: mirror simulation steps as fast as fence + lock allow.
  {
    ...
    SendMessageW(hFps, CB_ADDSTRING, 0, (LPARAM)L"Parity (uncapped)");
...
LRESULT DisplaysWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam) {
  ...
  if (code == BN_CLICKED) {
    ...
    case IDC_MW_DISP_HUD_DISABLE:
      ...
      WritePrivateProfileIntW(bChecked ? 1 : 0, L"bDisableMirrorHud",
                              p->GetConfigIniFile(), L"Settings");
      return 0;
